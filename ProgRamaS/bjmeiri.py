...
def salvar_dados_editados_tabela_funcionarios():
    ...
    atualizar ="UPDATE cd_func SET nome = '{}', descricao = '{}', marca = '{}', tamanho = '{}', preco_venda = '{}', preco_compra = '{}' where id = '{}' ".format (nomel,descl,marcl,tamanl, precvl, preccl, codl)
    cursor.execute(atualizar)
...
def pesquisar_dados():
    cursor = connect.cursor()
    linha = cd_vendas.id.text()
    cursor.execute("Select id from prod")
    cursor.fetchall()
    cursor.execute("Select * from prod where id = '{}'". format(str(linha)))
    busca = cursor.fetchall()
   
    cd_vendas.id.setText(str(busca[0][0]))
    cd_vendas.quant.setText(str(busca[0][1]))
    #cd_vendas.unitario.setText(str(busca[0][3]))
   
   
def select_vendas():
    id = cd_vendas.id.text()
    descricao = cd_vendas.descricao.text()
    quantidade = cd_vendas.quant.text()
    valor_uni = cd_vendas.valor_uni.text()
    total= cd_vendas.total.text()
   
    inserir= "Insert into cd_vendas (id, descricao, quantidade, valor_unitario, total) values (%s, %s, %s, %s,%s)"
    campos= (id,descricao, quantidade, valor_uni,total)
   
    cursor = connect.cursor()
    cursor.execute(inserir,campos)
    connect.commit() # salvar os dados que o usuario inseriu
   
    cursor.execute ("Select * from vendas where ordem = '{}'". format(id))
    dados = cursor.fetchall()
    cd_vendas.tableWidget.setRowCount(len(dados))
    cd_vendas.tableWidget.setColumnCount(5)
   
    for i in range (0,len(dados)):
        for j in range (0,5):
            cd_vendas.tableWidget.setItem(i, j, QtWidgets.QTableWidgetItem(str(dados[i][j])))
...
cd_vendas=uic.loadUi('tela_venda.ui') # Inicializador da tela de cadastro de vendas
...
vendas=uic.loadUi('tela_venda.ui') # Inicializador para iniciar tela vendas

alter_prod=uic.loadUi('alter_prod.ui') # Inicializador para iniciar tela alteraçao de produto

edit_prod=uic.loadUi('edit_prod.ui') # Inicializador para iniciar tela
 
alter_forne=uic.loadUi('alter_forne.ui') # Inicializador para iniciar tela

edit_forne=uic.loadUi('edit_forne.ui') # Inicializador para iniciar tela

alter_func=uic.loadUi('alter_func.ui') # Inicializador para iniciar tela

edit_func=uic.loadUi('edit_func.ui') # Inicializador para iniciar tela

edit_meiri=uic.loadUi('edit_meiri.ui') # Inicializador para iniciar tela

alter_meiri=uic.loadUi('alter_meiri.ui') # Inicializador para iniciar tela
...
cd_vendas.pesquisa.clicked.connect(pesquisar_dados)
cd_vendas.voltar.clicked.connect(chama_opçoes)
cd_vendas.finalizar.clicked.connect(chama_opçoes)
...
cd_user.inscrevase.clicked.connect(chama_L_user) # Botão de inscrever-se da tela de login
...
cd_opções.func.clicked.connect(bt_pesquisar)

cd_opções.forne.clicked.connect(chama_forne)

cd_opções.tb_func.clicked.connect(alteracao_tabela_funcionarios)

cd_opções.tb_cliente.clicked.connect(alteracao_tabela_cliente) # botão de 'tabela de clientes' na tela de opções

cd_opções.tb_prod.clicked.connect(alteracao_tabela_produtos)
